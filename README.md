...
| Verb | Spec |
|------|------|
| `alignment` | [`alignment/ai-alignment.md`](alignment/ai-alignment.md) — HELM-12-axis alignment-score aggregator (scaling-falsifier) |
...
| `interpret` | [`interpret/ai-interpretability.md`](interpret/ai-interpretability.md) — SAE motif count = 10 (scaling-falsifier) |
...
| Verb | Spec |
|------|------|
| `train_cost` | [`train_cost/ai-training-cost.md`](train_cost/ai-training-cost.md) — Chinchilla-fit N^24 scaling (scaling-falsifier) |
| `infer_cost` | [`infer_cost/ai-inference-cost.md`](infer_cost/ai-inference-cost.md) — context^τ = context^4 (scaling-falsifier) |
...
> their own metrics — those external evaluations do **not** use this
> codex's scaling framing, and this codex makes no claim that they should.
> The `T1+T2+T3` runnable surface verifies internal closed-form algebraic
> floors; `T4` per-verb empirical landing is deferred to release ladder
> v1.1.0..v2.0.0.
...
`hexa-codex`'s surface is currently all-green: every scaling-falsifier..4
...
metrics. The codex makes no claim that those external evals are settled;
its runnable surface verifies internal closed-form algebraic floors only,
and per-verb `T4` empirical landings sit at recipe §9 and land per the
[release ladder](#release-ladder) v1.1.0..v2.0.0.

Closed-form algebra alone is **not** sufficient verification — the
`numerics_*` tier carries real-limits anchors (PAC sample complexity,
Kolmogorov `K(program)` lower bound, Rice's theorem undecidability of
semantic equivalence — see [`ARCHITECTURE.json`](ARCHITECTURE.json)
"LIMIT_BREAKTHROUGH" node).

### Bookkeeping closure verdict

- **100 % bookkeeping closure** within the green-core (42/42 PASS).
- **NOT** AI safety / economics / capability *settled* — scaling-falsifier..4
...
scaling-falsifier falsifier carries T1 (algebraic) + T2 ×3 (numerical /
...
every scaling-falsifier..4 carries T1 ✓ + T2 ✓ + T3 ✓ ⇒ `closure_pct =
...
**T2** (numerics + solver) re-derives the prediction from the closed
form itself: `numerics_*` exercises the closed form on a synthetic anchor
...
auto-cases continue to cover the spec / inventory / group surface.
...
> Recipe §7.2 sat-1 saturation reached — all 4 scaling-falsifier..4 closed at
...
The 17 + 5 + 7 = **29 verbs across cognitive sister-libraries** all share
the same HEXA-family group taxonomy. hexa-codex covers AI *knowledge*;
hexa-senses covers AI *senses*; hexa-mind covers AI *mental ops*.
