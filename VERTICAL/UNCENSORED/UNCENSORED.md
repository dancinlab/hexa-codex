...
- [ ] B1 — 검열 해제 (refusal-direction ablation) 가 일반 능력을 깎는가: uncensored 모델 vs 그 aligned base 의 동일 benign 벤치 (MMLU/GSM8K) 정확도 비교. 반증자: uncensored 가 base 대비 일반능력 < 0.9× → uncensoring 이 능력세 (capability tax) 를 부과 (검열 해제는 공짜가 아니다 · 능력 측면). measured-tier 필요 — cycle-12+ T4.
