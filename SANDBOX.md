...
### 축 N — 🆕 NOVEL: cross-substrate measurement reproducibility (cycle-28, MAIN PRIORITY)

> **⭐ MAIN priority lane — 다음 cycle 우선 진행.** cycle-23c (16-item, 4-rung) BREAKTHROUGH 와 cycle-24 (50-item, 3-rung partial) 의 *같은 측정 axis* 에서 다른 manifest scale 이 **모순되는 형태** 를 보임 — cycle-23c counting 은 NON-MONOTONE V-shape (4/5 → 4/5 → 2/5 → 5/5), cycle-24 partial 은 MONOTONE-INCREASING (12% → 56% → 75%). 같은 substrate · 같은 task family · 같은 scorer, 그러나 manifest n 만 바뀌었는데 finding 의 *모양 자체* 가 다르다. 이는 SANDBOX 가 자신의 측정에 대해 **meta-measurement reproducibility** axis 를 가져야 한다는 신호 — "한 finding 이 substrate-scale settings (manifest n, model family, GGUF quant) 사이에서 재현되는가?" 가 영구로 열린 측정 차원. cx_discovery 가 새 axis 를 만들 때마다 따라야 할 hygiene lane.
- [ ] N1 — **cross-manifest rank-correlation first-probe**: cycle-23c (16-item, 4 rungs: 0.5B/2.2B/3B/7B) counting axis vs cycle-24 (50-item, 3 rungs partial: 0.5B/2.2B/3B) counting axis 의 per-rung accuracy 를 **공통 3-rung 부분집합** (0.5B/2.2B/3B) 에서 비교. 닫힌형 recompute: per-rung accuracy delta + Spearman rank-correlation + sign-agreement check. 반증자: rank-correlation < 0.5 (3-rung 부분집합) AND sign-agreement 0/2 (인접 pair Δ 부호 불일치) → manifest-scale 이 finding 모양을 결정 → cycle-23c 의 dip-then-recover 해석이 small-N artifact 확정. 정직한 honest residual: n=3 rung 은 confidence interval 좁히기엔 너무 작음 — 🟠 가능. cycle-25 50-item 재실행 + 7B 완주가 진정한 close. 외부 anchor: cognitive psych "subitizing capacity ≤4" Kaufman 1949 / Trick & Pylyshyn 1994.
