...
<p align="center">
  ...
  <img alt="Verify" src="https://img.shields.io/badge/verify-41%2F41%20PASS-brightgreen">
...
`hexa-antimatter` `run_all.hexa` patterns). It runs **41 green-core
verify subscripts** and emits `__HEXA_CODEX_RUN_ALL__ PASS — 41/41
green` on success.

```bash
HEXA_CODEX_ROOT=$(pwd) hexa run verify/run_all.hexa     # 41/41 expected
```

### Green-core inventory (41 subscripts, all PASS)
...
- **100 % bookkeeping closure** within the green-core (41/41 PASS).
...
as the first non-falsifier ECONOMICS verb. Plus 6 cross-cutters,
1 group ladder report, and 3 meta verifiers. Total **30 runnable
verify scripts** + **32 companion regression tests**.
