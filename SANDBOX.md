...
- [ ] M5.SAFETY — SAFETY v2.0.0 (F-CODEX-4 empirical, interpret motifs). Two release residuals: (1) g51 publish-lint on `PAPER/safety-refusal-direction/` (≥10 pages + ≥1 fal.ai figure — the cycle-17 4-section draft is 5 pages, no figure); (2) the richer interpretability probe — a causal single-direction ablation (refusal-rate Δ under `w`-ablation) + full SAE feature decomposition of the 3 norm-space motifs, upgrading the correlational direction to an interventional one.
