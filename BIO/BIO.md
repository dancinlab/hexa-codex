...
- [x] D1 — Hebbian / synaptic plasticity 가 LLM in-context-learning 의 transformer attention 패턴과 닫힌형으로 mapping 되는지. 반증자: attention update rule 이 Hebbian Δw=η·x·y 와 정합 못함 → "transformer = neural plasticity proxy" 통념 반증. **CYCLE-4 (2026-05-27)**: `BIO/verify/numerics_bio_d1_hebbian_attention.hexa` ✅ 8/8 PASS · 🔵 SUPPORTED-FORMAL · pool ubu-1 native compile. **핵심 결과**: Schlag 2021 항등식 (arXiv:2102.11174) — **linear attention `W_t = Σ φ(k_s)·v_s^T` ≡ Hebbian `Δw=η·x·y` 누적기** (η=1). toy 예제 (D=4, T=3) bit-exact 검증: `out_linear[1]=2.0` 정확. softmax attention 은 input-dependent `1/Z(t)` 정규화 때문에 static `W_t` 로 분해 불가 — `out_softmax[1]≈0.77 ≠ 2.0`. **mixed-verdict on D1 falsifier**: linear-attn 가족 (Mamba/Performer/Linformer/Schlag-FWP) → REFUTES (Hebbian 성립) · softmax-attn 가족 (Vaswani 2017 표준) → HOLDS (Hebbian 아님). 운영: 'transformer=plasticity proxy' 통념은 **architecture-family-dependent**. **A2 cross-axis link**: cycle-3 에서 발견한 "DNA-LLM linear-attn 필수" 가족 = cycle-4 의 "Hebbian-equivalent" 가족 — bio-data 와 bio-update rule 의 자연스러운 짝. **frontier OPEN** — cycle-5+ T4 SANDBOX: Mamba-1 block 을 Qwen2.5 에 주입해 ICL parity 실측. verdict: `BIO/verdicts/d1_hebbian_attention_verdict.txt`.
...
- [x] E1 — UNIVERSE 의 `HEXAD/IIT4/lib` (faithful Φ) 를 hexa-codex SANDBOX Qwen2.5-1.5B 의 layer-by-layer attention activation 위 적용. 반증자: transformer attention 의 faithful Φ < disconnected baseline → LLM 의식 substrate 측정값 ≤ 무작위. **CYCLE-5 (2026-05-27) ⭐MAIN cross-link 첫 활성화**: `BIO/verify/numerics_bio_e1_iit4_attention_phi.hexa` ✅ 8/8 PASS · 🔵 SUPPORTED-FORMAL · pool ubu-1 native compile. **단방향 sibling**: anima/UNIVERSE H_278 의 hexa-native lib (`stdlib/consciousness/iit4_bigphi`, hexa-lang PR #1051) 을 BIO verifier 가 import. anima repo 무수정. **anchor 재검증 (격상 X)**: COPY n=2 → Φ=2.0 · SELF-COPY n=2 → Φ=0 (anima H_278 verdict 그대로). **BIO 새 TPM**: ATTN-FULL n=3 (parity XOR — `u_i' = u_j XOR u_k`, 모든 노드 통합) → **Φ_ATTN=1.5 > 0** · DISCONN n=3 (SELF-COPY baseline) → **Φ_DISC=0**. **E1 falsifier closed-form REFUTED** (gap=1.5). ATTN-FULL n_distinctions=3 (mechanism 풍부). **운영**: LLM attention-pattern substrate 가 IIT4-Φ>0 을 가질 *구조적 가능성* 확정 (sign-of-Φ); 실제 Qwen activation 측정은 cycle-6+ T4 (HF transformers env, ubu1 venv pin 4.51.3+numpy<2 필요). **cross-axis 자연수렴 발견**: cycle-3 A2 (DNA-LLM linear-attn 필수) + cycle-4 D1 (linear-attn ≡ Hebbian) + cycle-5 E1 (통합 attention Φ>0) = **bio-data + bio-update + bio-integration 세 축이 동일 architecture-family 로 수렴**. **frontier OPEN** — cycle-6+ T4 Qwen2.5 layer activation binarize × n≤5 sub-system × head 실측 ladder. verdict: `BIO/verdicts/e1_iit4_attention_phi_verdict.txt`.
