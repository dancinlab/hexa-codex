...
- [ ] A1 — protein-sequence LLM (ESM family / ProtGPT2 family) 의 자체 scaling-law 가 dense-text LLM 의 Chinchilla-family 와 동일한지 closed-form 비교 (ECONOMICS C1 Hoffmann Lagrangian 의 BIO 적용). 반증자: protein-LLM 의 D/N 최적이 텍스트 Hoffmann α/β 와 ε > 0.05 어긋남 → BIO 데이터는 별도 scaling-law 가족.
- [ ] A2 — DNA-token / amino-acid-token 의 어휘 크기 효과 (text 50k vs protein 20-amino vs DNA 4-nt) 가 model capacity 요구에 미치는 닫힌형 영향. 반증자: 어휘 크기와 model capacity 가 무관 → tokenization 가 BIO scaling 에 영향 없음.
